...
function confirmDeletePayment(payment: any) {
  if (payment.status === 'paid' || payment.status === 'approved') {
    toastStore.showNotification('No se puede eliminar un link pagado.', 'warning')
    return
  }
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="p in payments" :key="p._id">
                ...
                <td>
                  <span :class="['badge',
                    p.status === 'paid' || p.status === 'approved' ? 'badge-success' :
                    p.status === 'pending' ? 'badge-warning' :
                    p.status === 'canceled' ? 'badge-danger' : 'badge-info'
                  ]">
